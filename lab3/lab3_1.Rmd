...
## Working dictories
```{r}
getwd() ##by the default you ar in the dictory that you first opened
```
...
A numeric vector.
```{r}
my_vector <- c(10, 20, 30) #this makes a new vctor
```
...
```{r}
hbirds <- tibble(Sex,Length,Weight) #each vector is a column(variable)
```
