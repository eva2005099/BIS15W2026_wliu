...
Yes, it’s tidy: each row is one observation (a site on a date), each column is a variable. It is in long format (not wide)..
...
```{r}
names(sydneybeaches_wide_summary)
```
...
Where were the five cleanest beaches on average throughout the entire sampling period (2013-2018)?

```{r}
clean_beaches <- 
  sydneybeaches_long %>% 
  group_by(site) %>% 
  summarize(overall_avg_pollution = mean(enterococci_cfu_100ml, na.rm = TRUE)) %>% 
  arrange(overall_avg_pollution) %>%
  head(5)
  clean_beaches
```
I calculated the average enterococci concentration at each beach across all years and then selected the five beaches with the lowest overall average pollution.
