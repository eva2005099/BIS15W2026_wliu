...
gabon_factor %>% 
  ...
  geom_boxplot()+
  labs(title="How does vegetation richness compare among land use types?")
```
...
```{r}
gabon_factor %>% 
  mutate(average_diversity_all_species=mean(diversity_all_species),.by=land_use) %>% 
  select(land_use,average_diversity_all_species) %>% 
  arrange(desc(average_diversity_all_species)) %>% 
  distinct()
```
