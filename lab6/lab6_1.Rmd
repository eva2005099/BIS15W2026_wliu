...
```{r}
library("tidyverse")
library("janitor") #this will help wth cleaning
...
penguins %>% 
  ...
  geom_point()+
  labs(title="Relationship between Body Mass and Flipper Length among Different Species")+
...
```{r}
penguins %>% 
  distinct() #in this case, no duplicate
```
...
```{r}
mammals %>% 
  mutate(across(c("order","family"),tolower)) #work specific col
```

This will change all columns to lower case. But, notice what happens to numeric columns.
```{r}
mammals %>% 
  mutate(across(everything(),tolower)) #this approach will convert all numeric to characteristics
```
