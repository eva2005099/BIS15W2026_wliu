...
table_A <- 
  read_csv("data/table_A.csv")
table_A
```

```{r}
table_B <- 
  read_csv("data/table_B.csv")
table_B
```
...
```{r message=FALSE, warning=FALSE}
spider_records <- 
  read_csv("data/spider_records.csv") %>% 
  clean_names()
```

```{r, message = FALSE}
spider_locations <- 
  read_csv("data/spiders_locations.csv") %>% 
  clean_names()
```

2. Join the files using an appropriate key and join type.
```{r}
spider_with_locs <- left_join(spider_locations,spider_records,by="accession")
```

```{r}
names(spider_with_locs)
```
...
```{r}
# write.csv((spider_with_locs,file="spiders_with_locus.csv",row.names=FALSE))
```
