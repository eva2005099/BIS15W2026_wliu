...
```{r}
amniota %>% 
  replace_with_na(replace=list(female_maturity_d="-30258.7",
                               adult_body_mass_g="0",
                               birth_or_hatching_weight_g="0.000e+00",
...
6. Calculate the number of NAs in the `egg_mass_g` column sorted by taxonomic class; i.e. how many NA's are present in the `egg_mass_g` column in birds, mammals, and reptiladult_body_mass_ges? Does this results make sense biologically? How do these results affect your interpretation of NA's?
```{r}
amniota %>% 
  distinct(class)
```
...
amniota %>% 
  ...
  miss_var_summary()
```
Yes, it make sense, because for mammals, the 100% missing rate in egg_mass_g is biologically expected because they don't lay eggs.
## Knit and Upload
